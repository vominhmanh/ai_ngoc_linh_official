...
# ==================== CHAT COMPLETIONS ====================
def chat_completion(message: str, history: List, model: str, temperature: float, max_tokens: int,
                   base_url: str, api_key: str):
    """Handle chat completion requests with streaming"""
    try:
        logger.info(f"Chat completion - Model: {model}, Temp: {temperature}, Max tokens: {max_tokens}")
...
"""
PROMPT1 = """
Generate text with 2 lines: "Nuôi linh vật nhỏ - Tiền lớn bất ngờ". engaging and inspiring marketing copy for a banking or savings campaign
Output only Vietnamese text, no image to insert to another image.
"""
...
def generate_image(
    ...
    api_key: str="sk-ug1poexeERrICjPNvLSooQ"
) -> Tuple[Optional[str], str]:
    """Generate image using /images/generations endpoint"""
    logger.info(f"Image generation - Aspect: {aspect_ratio}")
    ...
    temp_path = f"generated_image_{int(time.time())}.png"
...
def generate_image_chat(    prompt: str=PROMPT1,
    base_url: str="https://api.thucchien.ai",
    api_key: str="sk-ug1poexeERrICjPNvLSooQ"
    ) -> Tuple[Optional[str], str]:
    """Generate image using chat completion API"""
    try:
        url = f"{base_url}/chat/completions"
        headers = {
            "Content-Type": "application/json",
        ...
        data = response.json()
        base64_string = data["choices"][0]["message"]["images"][0]["image_url"]["url"]
...
def edit_image(prompt: str, image_file: Optional[str], aspect_ratio: str, base_url: str, api_key: str) -> Tuple[Optional[str], str]:
